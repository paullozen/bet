    ...
    await csvMutex.runExclusive(async () => {
        try {
            ...
            if (fs.existsSync(csvPath)) {
                ...
                if (lines.length > 1) {
                    // Detect headers to see if we need to migrate on the fly or just push
                    // But assume migration script ran, so headers likely exist. 
                    // To be safe, we rebuild records fully.
                    for (let i = 1; i < lines.length; i++) {
                        const vals = lines[i].split(',');
                        if (vals.length >= 5) { // At least 5 basic cols
                            records.push({
                                Data: vals[0],
                                Competição: vals[1],
                                Hora: vals[2],
            ...
            if (existingIndex !== -1) {
                ...
                if (ambosMarcam) {
                    record['Ambos Marcam'] = ambosMarcam;
                }
            } else {
                record = {
                    Data: dateStr,
            ...
            // We need to calculate patterns for THIS record (and potentially re-calc others if we wanted to be robust, but usually we just append)
            // But if we just appended, we look back.
            
            // Helper to find result at specific time
            const getResultAt = (cName, h, m) => {
                const r = records.find(rec => 
                    rec['Competição'] === cName && 
                    parseInt(rec['Hora']) === parseInt(h) && 
                    parseInt(rec['Minuto']) === parseInt(m)
                );
                return r ? r['Ambos Marcam'] : null;
            };

            const currentMins = hour * 60 + minute;
            const currentRes = record['Ambos Marcam'];

            if (currentRes) {
                for (let n = 1; n <= 5; n++) {
                    const offset = (n + 1) * 3;
                    const targetMins = currentMins - offset;
                    // Handle day wrap? No, simplistic for now (same day/file)
                    if (targetMins < 0) continue; 
                    
                    const tH = Math.floor(targetMins / 60);
                    const tM = targetMins % 60;
                    
                    const targetRes = getResultAt(compName, tH, tM);
                    if (targetRes) {
                         record[`${n}x`] = (currentRes === targetRes) ? 'Sim' : 'Não';
                    }
                }
            }
            // --------------------------------
